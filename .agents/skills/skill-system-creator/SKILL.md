...
Skill creation follows a **Spec → Test → Develop** workflow:

1. Understand the skill with concrete examples
2. Create behavior spec (`SKILL.spec.yaml` via `skill-system-behavior:create-spec`)
3. Validate the spec (`skill-system-behavior:validate-spec`)
4. Define acceptance tests (structural + behavioral, from spec)
5. Initialize the skill (run init_skill.py)
6. Implement the skill (resources + SKILL.md — make tests pass)
7. Verify structural tests (`skill-system-behavior:verify-structural`)
8. Generate behavior contract (`skill-system-behavior:generate-contract`)
9. Package the skill (run package_skill.py)
...
- **Contract-first**: forces clear operation boundaries before coding
- **Testable**: acceptance criteria are executable, not aspirational
- **Prevents scope creep**: spec defines what's in, everything else is out
- **Generated docs**: behavior contracts (Mermaid DAGs) are derived from specs, not hand-written
