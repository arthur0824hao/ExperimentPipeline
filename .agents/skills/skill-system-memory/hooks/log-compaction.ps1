...
$logPath = Join-Path $env:USERPROFILE '.config\opencode\skill-system-memory\compaction-events.jsonl'
...
SELECT store_memory(
  ...
  jsonb_build_object(
    ...
    'source', 'opencode-hook-compaction'
  ),
  'opencode-hook',
  :'session_id',
  7.0
...
try {
  ...
  Write-JsonlLine -Path $logPath -Obj @{
    event = 'session.compacting.error'
    time_utc = $timestamp
    session_id = $sessionId
