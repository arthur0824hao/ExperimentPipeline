...
log_path="$HOME/.config/opencode/skill-system-memory/compaction-events.jsonl"
...
printf '{"event":"session.compacting","time_utc":"%s","session_id":"%s","trigger":"%s","cwd":"%s","transcript_path":"%s","transcript_tail_b64":"%s"}\n' \
...
sql+=$'    \x27source\x27, \x27opencode-hook-compaction\x27\n'
sql+=$'  ),\n'
sql+=$'  \x27opencode-hook\x27,\n'
sql+=$'  :\x27session_id\x27,\n'
sql+=$'  7.0\n'
sql+=$');'

psql -w -h localhost -p 5432 -U "${PGUSER:-$(whoami)}" -d agent_memory \
  -v ON_ERROR_STOP=1 \
