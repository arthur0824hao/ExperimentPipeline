...
if ($PluginOnly) {
  ...
  try {
    ...
    $helperSrc = Join-Path $PSScriptRoot '..\plugins\runtime_sync.js'
    $helperSrc = (Resolve-Path -LiteralPath $helperSrc).Path
    $dst = Install-OpenCodePlugin -PluginName 'skill-system-memory.js' -SourcePath $src
    $helperDst = Install-OpenCodePlugin -PluginName 'runtime_sync.js' -SourcePath $helperSrc
    $record.notes += "OpenCode plugin installed: $dst"
    $record.notes += "OpenCode helper installed: $helperDst"
