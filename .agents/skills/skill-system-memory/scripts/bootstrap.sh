...
if [ "$selected_pgvector" = "true" ] && command -v psql >/dev/null 2>&1; then
  psql -w -h localhost -p 5432 -U "${PGUSER:-$(whoami)}" -d agent_memory -v ON_ERROR_STOP=1 -c "CREATE EXTENSION IF NOT EXISTS vector;" >/dev/null 2>&1 || true
fi

if [ "$selected_opencode_plugin" = "true" ]; then
  src_dir="$(cd "$(dirname "$0")/.." && pwd)"
  src="$src_dir/plugins/skill-system-memory.js"
  helper_src="$src_dir/plugins/runtime_sync.js"
  dst_dir="$HOME/.config/opencode/plugins"
  mkdir -p "$dst_dir"
  if [ -f "$src" ] && [ -f "$helper_src" ]; then
    cp -f "$src" "$dst_dir/skill-system-memory.js"
    cp -f "$helper_src" "$dst_dir/runtime_sync.js"
    echo "Installed OpenCode plugin files to $dst_dir"
    echo "Restart OpenCode to load plugin"
  else
    echo "Plugin source not found: $src or $helper_src" >&2
  fi
fi
