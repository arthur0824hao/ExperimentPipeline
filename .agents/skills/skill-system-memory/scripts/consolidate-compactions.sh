...
events_path="${EVENTS_PATH:-$HOME/.config/opencode/skill-system-memory/compaction-events.jsonl}"
out_dir="${OUT_DIR:-$HOME/.config/opencode/skill-system-memory/compaction-daily}"
...
generate_daily_files_jq() {
  jq -r 'select(.event=="session.compacting" and .time_utc) | .time_utc[0:10] as $d | [$d, (.session_id//"unknown"), (.trigger//"unknown")] | @tsv' "$events_path" \
    | awk -F"\t" '
      {
        day=$1; sess=$2; trig=$3;
        c[day]++;
...
with open(events_path, "r", encoding="utf-8") as f:
    for line in f:
        ...
        ev = obj.get("event")
        t = obj.get("time_utc")
        if ev != "session.compacting" or not t:
            continue
        day = str(t)[:10]
