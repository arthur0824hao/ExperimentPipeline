...
- Current soul state YAML (`category='soul-state'`) for the user (or use `SELECT * FROM get_soul_state('{user}');` for typed access)
- One facet YAML (`category='insight-facet'`) for the user (or use `SELECT * FROM get_recent_facets('{user}', 1);` for typed access)
...
Store via `upsert_soul_state()` (writes to both `soul_states` and `agent_memories`):

```sql
SELECT upsert_soul_state(
  '{user}',
  '{soul-state YAML}',
  '{personality JSONB}',
  '{emotion JSONB}',
...
This writes to both `soul_states` (typed) and `agent_memories` (general log).

(Legacy fallback: `SELECT store_memory('semantic', 'soul-state', ARRAY['user:<user>', 'matrix'], 'Soul State: <user>', '<soul-state YAML>', '{"total_insights": <N>, "last_updated": "<iso>"}', 'insight-agent', NULL, 9.0);`)
