...
Artifacts involved:
- Prompts: `prompts/evolution-planning.md`
- Schemas: `schema/evolution-plan.yaml`, `schema/evolution-snapshot.yaml`
- Storage: Postgres `agent_memories` table via `store_memory()`
- Output file: `../skill-system-soul/profiles/{user}.md`
...
```sql
SELECT * FROM get_soul_state('{user}');
```
(typed table; also available via `agent_memories` WHERE `category='soul-state'`)
...
```sql
SELECT * FROM get_recent_facets('{user}', 50);
```
(typed table)

## Step 3: Load current soul profile markdown
Read in this order:
1. `../skill-system-soul/profiles/{user}.md` if it exists
2. Otherwise `../skill-system-soul/profiles/balanced.md`

## Step 4: Generate an evolution plan
Use the prompt:
- `prompts/evolution-planning.md`
...
Output required:
- A single YAML document that matches `schema/evolution-plan.yaml`
...
Construct a snapshot YAML matching `schema/evolution-snapshot.yaml`.
