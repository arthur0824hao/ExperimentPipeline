...
- Current soul state (Layer 2):
  - latest `category='soul-state'` for user (or `get_soul_state('{user}')`)
- Recent facets:
  - last 50 memories where `category='insight-facet'` and `tags` contains `user:<user>` (or `get_recent_facets('{user}', 50)`)
...
Store via `upsert_soul_state()` (writes to both `soul_states` and `agent_memories`, same pattern as `scripts/update-matrix.md`).
